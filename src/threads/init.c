...
int pintos_init(void) {
  ...
  if (*argv != NULL) {
    ...
    while (1) {
      printf("PKUOS>");
      char buffer[128]; // 128 bytes buffer
      int pos = 0;      // position in buffer

      // read a line from the keyboard and store it in buffer
      while (1) {
        ...
        // valid characters
        if (key >= 32 && key <= 126) { // valid ASCII character
          if (pos < sizeof(buffer) - 1) {
            buffer[pos++] = key;
            putchar(key); // echo the character to the screen
          }
        }
      }
      ...
      if (strcmp(buffer, "whoami") == 0) {
        printf("2025/1/8 finish the shell kernel.\n");
      } else if (strcmp(buffer, "exit") == 0) {
        printf("Exiting shell kernel...\n");
        break;
      } else {
