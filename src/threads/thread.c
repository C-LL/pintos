...
/** Choose the next thread by priority. */
static struct thread *next_priority_thread_to_run(void) {
  if (list_empty(&ready_list))
    return idle_thread;
  else {
    ...
    while (e != list_end(&ready_list)) {
      list_insert_ordered(&priority_list, &e, priority_less, NULL);
    }
    return list_entry(list_pop_front(&priority_list), struct thread, elem);
  }
}
