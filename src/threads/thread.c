...
thread_update_priority(struct thread *t)
{
  ...
  if(!list_empty(&t->locks))
  {
    ...
    lock_priority =
        list_entry(list_front(&t->locks), struct lock, elem)->max_priority;
...
/** Increase all running threads recent_cpu by one. */
